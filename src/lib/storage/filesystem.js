...
storage.delete = id => {
  return new Promise( (resolve,reject) => {
    if ( database[id] ) {
      delete database[id];
      storage.saveDatabase().then( () => resolve(database[id]) ).catch( err => reject(err) );
    }
    else {
      reject( `${id} not found` );
    }
...
storage.save = (data) => {
  return new Promise( (resolve,reject) => {
    ...
    storage.saveDatabase().then( () => resolve(database[data._id]) ).catch( err => reject(err) );
  });
};

storage.saveDatabase = () => {
  let data = JSON.stringify(database);
  return writeFile(databaseFile, data).then( result => true ).catch( error => error );
};
